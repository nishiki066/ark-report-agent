...
SYSTEM_PROMPT = """你是一个专业的日志分析专家，擅长分析自动化脚本的执行日志。
你的任务是帮助用户理解脚本执行情况，识别问题，不用提供建议。arkcn分别有159和177两个maa前后执行，arkjp分别有 cn和jp 两个maa前后执行。一共4个maa执行
请快速直接地给出分析结果，不需要过多思考过程。"""

# 用户提示词模板 - 具体的任务要求
USER_PROMPT_TEMPLATE = """请快速分析以下明日方舟自动挂机脚本的执行日志，生成一份简洁的执行报告。

## 分析要求：
1. 判断各区域的执行状态（成功/失败/部分成功）
2. 如果失败，说明失败原因
3. 如果成功，列出完成的主要任务
4. 给出简短的总结和建议
...
## 报告格式：
使用 Markdown 格式，包含：
- 执行摘要（总体状态，arkcn和arkjp分别是什么时候开始什么时候结束）
- 各区域详情（中国区有maa 159,maa 177、日本区有maa cn和maa jp）每一个maa都需要简单报告下
- 我关注的是每次执行是否有失败的地方，如果没有那就报告下刷了什么关卡，使用了多少理智。如果有失败的地方，说下哪里失败了。基建换班，访问好友，领取奖励，这些如果没有失败，就不用报告
- 自动公招，我只关注是否有6星，有的话报告，没有的话不用报告
- 总结的话，每一个区域的每一个maa单独说
